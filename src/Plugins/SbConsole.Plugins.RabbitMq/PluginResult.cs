...
/// <summary>
/// This plugin's own lightweight result type, mirroring SbConsole.Plugins.Aws.PluginResult
/// without depending on it -- plugins reference only SbConsole.Sdk. Routes exception failures
/// through FriendlyRabbitError (not the plain SbConsole.Sdk.FriendlyError) so management-API and
/// RabbitMQ.Client exceptions get their fixed readable messages.
/// </summary>
public sealed class PluginResult
{
    ...
    public static PluginResult Fail(Exception ex) => new(false, FriendlyRabbitError.From(ex));
}
...
public sealed class PluginResult<T>
{
    ...
    public static PluginResult<T> Fail(Exception ex) => new(false, default, FriendlyRabbitError.From(ex));
}
